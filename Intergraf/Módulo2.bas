...
Do Until Cells(LIN, 24) = ""
    If Cells(LIN, 24) = Cells(LIN + 100, 24) Then
        Range(Cells(LIN, 24), Cells(LIN + 100, 24)).Delete Shift:=xlUp
        GoTo FMLP
        ElseIf Cells(LIN, 24) = Cells(LIN + 10, 24) Then
            Range(Cells(LIN, 24), Cells(LIN + 10, 24)).Delete Shift:=xlUp
            GoTo FMLP
            ElseIf Cells(LIN, 24) = Cells(LIN + 1, 24) Then
                Cells(LIN + 1, 24).Delete 'Shift:=xlUp
                GoTo FMLP
    End If
    LIN = LIN + 1
FMLP:
...
Do Until Cells(LIN, 26) = ""
        'If Cells(LIN, 26) = Cells(LIN + 10, 26) Then
        '    Range(Cells(LIN, 26), Cells(LIN + 10, 26)).Delete 'Shift:=xlUp
        '    GoTo FMLP
        '    Else
        If Cells(LIN, 26) = Cells(LIN + 1, 26) Then
                Cells(LIN + 1, 26).Delete 'Shift:=xlUp
                GoTo FMLP
        End If
    LIN = LIN + 1
FMLP:
...
For COR_ATI = 4 To Mid(Cells(1, 1), 4, 2) + 3
If Cells(COR_ATI, 2).Interior.ColorIndex = 15 Then
    ...
    Cells(COR_ATI, 5).Interior.ColorIndex = 2
End If
If PAIS = Cells(COR_ATI, 2) And TELE = Cells(COR_ATI, 3) Then
    ...
    Cells(COR_ATI, 5).Interior.ColorIndex = 15
End If
